...
public interface SolicitacaoViagemRepository extends JpaRepository<SolicitacaoViagem, UUID> {
    ...
    /**
     * Solicitações de um passageiro (aba "Minhas viagens"), com a linha e — quando
     * alocada — a viagem e seus relacionamentos carregados (evita N+1 e
     * {@code LazyInitializationException}). Mais recentes primeiro.
     */
    @Query("""
            select s from SolicitacaoViagem s
            join fetch s.linha li
            join fetch li.cidadeDestino
