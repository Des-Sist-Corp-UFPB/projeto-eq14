...
## `UsuarioServiceTest` — 19 cenários (SPEC-ACE-01/02, DT-02)
...
| com WhatsApp configurado, nasce `PENDENTE` e dispara OTP | SPEC-ACE-03 — conta só ativa após verificar |
...
| `completarPerfil` grava telefone e limpa `perfilIncompleto` | SPEC-ACE-02 — conta do Google sai do limbo |

## `ViagemServiceTest` — 15 cenários (SPEC-VIA-01/06)
...
| designar materializa `ROTINEIRA` com a origem da linha | SPEC-VIA-02 — a linha é template, a viagem é a instância |
...
## `SolicitacaoViagemServiceTest` — 18 cenários (SPEC-VIA-03/11)
...
| `solicitarSobDemanda` nasce `SOB_DEMANDA` + `PENDENTE` com destino/horário | SPEC-WPP-02 — pedido sem linha |
...
## `VerificacaoServiceTest` — 11 cenários (SPEC-ACE-03, TDD)
...
> Desde a SPEC-PLT-01, validade/tentativas/cooldown vêm do **feature toggle**. O teste injeta um
...
## `RecuperacaoSenhaServiceTest` — 9 cenários (SPEC-ACE-03)
...
## `ConviteServiceTest` — 11 cenários (#20, ADR-11, SPEC-ACE-03)
...
| **token de verificação de e-mail não pode definir senha** | SPEC-ACE-03 — finalidades não se misturam (seria escalonamento) |
...
## `LinhaProgramadaServiceTest` — 9 cenários (SPEC-VIA-02)
...
## `VeiculoServiceTest` — 8 cenários (SPEC-CAD-02, DT-04)
...
## `EnderecoServiceTest` — 4 cenários (SPEC-CAD-04)
...
## `IdentidadeOauthServiceTest` — 7 cenários (SPEC-ACE-02)
...
## `OnboardingServiceTest` — 3 cenários (SPEC-WPP-02)
...
`telefoneDeJid` (SPEC-WPP-01), `variantesTelefoneBr` (o 9º dígito — número antigo × novo) e
