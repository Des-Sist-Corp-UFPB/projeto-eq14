# SPEC-ACE-01 — Autenticação e Auto-cadastro
...
- **RN-AUT-02** — **E-mail e CPF são opcionais**, mas, quando informados, devem ser únicos
  entre usuários ativos (validado no cadastro — ver SPEC-CAD-01).
...
- Recuperação de senha (esqueci a senha) — **coberto por [SPEC-ACE-03](SPEC-ACE-03-verificacao-de-contato-e-recuperacao-de-senha.md)** (OTP via WhatsApp).
- Telas/redirecionamentos por papel após login (hoje todos caem em `/`).
- Verificação de e-mail/telefone (confirmação por link/código) — **coberto por [SPEC-ACE-03](SPEC-ACE-03-verificacao-de-contato-e-recuperacao-de-senha.md)** (telefone por OTP/WhatsApp; e-mail por link).
- Bloqueio por tentativas (rate limiting / lockout) — **parcialmente por [SPEC-ACE-03](SPEC-ACE-03-verificacao-de-contato-e-recuperacao-de-senha.md)** (lockout/throttle dos **códigos**); o lockout do **login por senha** segue fora do escopo.
