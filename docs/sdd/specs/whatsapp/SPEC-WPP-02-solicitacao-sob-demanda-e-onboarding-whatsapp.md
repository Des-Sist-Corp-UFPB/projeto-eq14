# SPEC-WPP-02 — Solicitação de transporte sob demanda e onboarding do passageiro por WhatsApp
...
| **Relacionada** | [SPEC-VIA-03](../viagens/SPEC-VIA-03-solicitacao-de-transporte.md) (contraparte por **linha programada**; realiza a **RN-SOL-08**), [SPEC-WPP-01](SPEC-WPP-01-integracao-whatsapp.md) (bot; **altera RN-WPP-05** e a exclusão §3.2), [SPEC-VIA-02](../viagens/SPEC-VIA-02-viagens-rotineiras-e-imprevistas.md) (viagem **IMPREVISTA**), [SPEC-CAD-04](../cadastros/SPEC-CAD-04-endereco-do-passageiro.md) (endereço de embarque), [SPEC-ACE-01](../acesso/SPEC-ACE-01-autenticacao.md) (RBAC) · **ADR-15** (proposta, §4) |

---

## 1. A lacuna que esta spec cobre

A SPEC-VIA-03 entregou a solicitação **ancorada numa linha programada**: o passageiro escolhe, num
...
1. **Login não deveria ser pré-requisito.** O passageiro do transporte municipal de saúde é, em
   ...
   **rejeita número desconhecido** (SPEC-WPP-01, RN-WPP-05) e manda procurar a secretaria. Isso trava a
...
SPEC-VIA-03). O modelo de **linha programada** da SPEC-VIA-03 **continua válido** e coexiste — esta spec
...
|---|---|---|
| Número desconhecido é **rejeitado**, sem criar cadastro | SPEC-WPP-01 RN-WPP-05 / §4.5 | Passa a **oferecer o auto-cadastro** guiado pelo bot; só cria conta com o consentimento e os dados mínimos (RN-ONB-01..03) |
| **Auto-cadastro por WhatsApp** listado como "não inclui" | SPEC-WPP-01 §3.2 | Passa a **estar no escopo** (esta spec) |
| Solicitação **exige** uma `linha_programada` (FK NOT NULL) | SPEC-VIA-03 V11 / RN-SOL-01 | Passa a existir a solicitação **sem linha** (sob demanda), com **destino + data + requisitos** próprios — RN-SOL-08 realizada |
| Alocação **deriva** da designação da viagem da linha | SPEC-VIA-03 RN-SOL-05 | Solicitação sob demanda **não tem linha**: exige **avaliação do gerente** (aprovar → designar viagem **IMPREVISTA** → alocar; ou **recusar**) |

Nada disso quebra a SPEC-VIA-03/10 já implementada: a tela web por linha e o fluxo de bot por linha
...
- Coleta guiada dos **dados mínimos para viajar** (D3): **nome completo**, **endereço de embarque**
  (onde o motorista busca — SPEC-CAD-04) e **CPF**.
...
- O gerente vê e completa/edita esse cadastro nas telas de usuário já existentes (SPEC-CAD-01).
...
- **Painel do gerente** para as solicitações sob demanda: listar pendentes (com as condições, para
  avaliar prioridade), **aprovar** (designa uma viagem **IMPREVISTA** — SPEC-VIA-02 — com veículo
...
- **Notificação do resultado** ao passageiro pelo canal WhatsApp — **só após a aprovação/recusa do
  gestor** é que o passageiro é avisado. Reusa o `NotificacaoService`/canal da SPEC-WPP-01.
...
- Opção do menu que permite ao usuário **só-WhatsApp** obter **acesso web**: o bot **reaproveita o
  fluxo de token de ativação** já existente (convite/token — ADR-11, SPEC-ACE-01) e envia um **link**
...
- Opção **3** do menu que **lista as linhas rotineiras ativas** (origem→destino, dias da semana e
  ...
  "Linhas disponíveis" da web (SPEC-VIA-03) — `SolicitacaoViagemService#linhasDisponiveis()`.
...
- **Assentos/capacidade** (`assentos_viagem`) e o encadeamento acompanhante↔titular — continua fora
  (como na SPEC-VIA-03 §2.2); a aprovação aqui **aloca** o passageiro à viagem, sem controle fino de lotação.
...
- **Auto-cadastro pela web** (o onboarding desta spec é **só** pelo WhatsApp; pela web o cadastro
  segue sendo do gerente — SPEC-CAD-01).
- **Roteirização** de embarque a partir do endereço estruturado (só se **registra** o local).
- Alteração do fluxo **por linha** da SPEC-VIA-03 (permanece intacto).
...
bot da SPEC-WPP-01. A tabela dormente `solicitacoes_transporte` (V2) fica formalmente **aposentada**
...
de solicitação, não no cadastro. O endereço reaproveita a estrutura da SPEC-CAD-04 (no mínimo
...
**0** Encerrar. (A opção "falar com a gestão" da SPEC-WPP-01 sai do menu; a etapa `HUMANO` permanece no
...
> aba web da SPEC-VIA-03). É **só leitura**: não muda o estado da conversa (permanece no `MENU`) e o pedido
...
- Onboarding **não cria tabela nova**: usa `usuarios` (**ATIVO, `hash_senha` NULL** — D2) + o
  `endereco` da SPEC-CAD-04 (embarque). "Acesso à plataforma" reusa `tokens_ativacao` (ADR-11).
...
## 6. Fluxo do bot (SPEC-WPP-01 estendida) — os dois cenários
...
- Máquina de estados persistida em `conversas_bot` (SPEC-WPP-01) ganha as novas etapas (novos valores de
  `EtapaConversa` + contexto de cadastro/pedido). Textos centralizados em `MensagensBot`.
- Aprovação/recusa pelo gerente **dispara notificação WhatsApp** ao passageiro (canal da SPEC-WPP-01).
...
| **RN-AVU-03** | **Aprovar** exige designar/associar uma viagem **IMPREVISTA** (SPEC-VIA-02) com **veículo adequado** (acessível/`AMBULANCIA` conforme as condições) → status `ALOCADA` + notifica. |
...
- Bot: sem rota web (entra pelo webhook da SPEC-WPP-01).
...
- **SPEC-VIA-03**: marcar **RN-SOL-08** como **realizada por SPEC-WPP-02**.
- **SPEC-WPP-01**: **RN-WPP-05** deixa de "rejeitar" e passa a "oferecer cadastro"; remover a exclusão de
  §3.2 (auto-cadastro) apontando para SPEC-WPP-02.
...
## 11. Coexistência com a SPEC-VIA-03 (resumo)

Dois caminhos, uma fila (se D1=A):

- **Por linha (SPEC-VIA-03)** — passageiro logado, tela web, escolhe linha existente; alocação automática
  na designação. Etiqueta **azul** (rotineira).
- **Sob demanda (SPEC-WPP-02)** — passageiro pelo WhatsApp (com onboarding), pede destino+data+requisitos;
...
4. Onboarding no bot + `OnboardingService` (cria PASSAGEIRO ATIVO sem senha; endereço da SPEC-CAD-04).
...
9. Atualizar SPEC-VIA-03/10, roadmap e CLAUDE.md.
...
flowchart LR
    ...
    subgraph BOT["Bot WhatsApp — SPEC-WPP-02"]
