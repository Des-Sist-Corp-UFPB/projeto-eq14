# SPEC-WPP-01 — Integração WhatsApp (conexão, envio, webhook e bot de atendimento)
...
| **Relacionada** | [SPEC-VIA-03](../viagens/SPEC-VIA-03-solicitacao-de-transporte.md) (o bot reaproveita o `SolicitacaoViagemService`), [SPEC-ACE-01](../acesso/SPEC-ACE-01-autenticacao.md) (identificação por telefone), ADR-11 (`NotificacaoService`), **ADR-14** (esta decisão) |
...
2. **Receber** mensagens via **webhook** e responder de forma automatizada — um **bot de
   ...
   pelo WhatsApp**, reaproveitando o `SolicitacaoViagemService` (SPEC-VIA-03).
...
- **Porta `ProvedorWhatsapp`** + adaptador **`EvolutionApiProvedor`** (`RestClient` do Spring),
  ...
  da SPEC-ACE-02): sem `EVOLUTION_URL`/`EVOLUTION_API_KEY`, o app sobe normalmente e o canal segue
  como stub.
...
- **Aprovação/recusa** de solicitações pelo gestor e **assentos/capacidade** (continuam como na SPEC-VIA-03).
- Envio de **mídia** (imagens/PDF de comprovante), listas/botões interativos do WhatsApp.
- ~~**Auto-cadastro** de passageiro pelo WhatsApp~~ → passou a ser **escopo da [SPEC-WPP-02](SPEC-WPP-02-solicitacao-sob-demanda-e-onboarding-whatsapp.md)** (implementado).
...
**Ativação condicional** (padrão da SPEC-ACE-02): o bean `EvolutionApiProvedor` só é criado com
...
- Respostas curtas, numeradas ("*Responda com o número da opção*"); entrada inválida repete a
  etapa com instrução.
- **Solicitar transporte** reaproveita **integralmente** o `SolicitacaoViagemService` (SPEC-VIA-03):
...
| **RN-WPP-05** | Identificação por **telefone normalizado** de usuário **ativo**. ~~Número desconhecido recebe orientação padrão, sem criar cadastro.~~ **Atualizada pela [SPEC-WPP-02](SPEC-WPP-02-solicitacao-sob-demanda-e-onboarding-whatsapp.md):** número desconhecido agora é **guiado ao auto-cadastro** (onboarding), criando um passageiro ATIVO sem senha. |
| **RN-WPP-06** | O bot age **em nome do passageiro identificado** e só sobre os dados dele (mesmo isolamento da RN-SOL-07); solicitações criadas passam pelas mesmas validações da SPEC-VIA-03 (RN-SOL-01..06). |
...
- **`BotAtendimentoServiceTest`** (Mockito, provedor mockado): fluxo completo de solicitação
  ...
  expiração (RN-WPP-07), regras da SPEC-VIA-03 propagadas como resposta amigável, isolamento
...
1. **Aprovação/recusa** pelo gestor com notificação WhatsApp do resultado (fecha o ciclo da SPEC-VIA-03).
