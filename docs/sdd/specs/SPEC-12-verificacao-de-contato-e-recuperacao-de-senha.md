# SPEC-12 — Verificação de contato e recuperação de senha
...
| **Relacionada** | [SPEC-01](SPEC-01-autenticacao.md) (login senha; realiza os itens da sua §9), [SPEC-08](SPEC-08-login-social-google.md) (Google/OIDC; e-mail já verificado pela Google), [SPEC-10](SPEC-10-integracao-whatsapp.md) (canal WhatsApp e a porta `ProvedorWhatsapp`), [SPEC-11](SPEC-11-solicitacao-sob-demanda-e-onboarding-whatsapp.md) (onboarding pelo bot; token "Acesso à plataforma") · **ADR-16** (proposta, §4) |

---

## 1. A lacuna que esta spec cobre

A **SPEC-01 §9** já lista, explicitamente, **três coisas fora do escopo** até aqui — e são exatamente
...
|---|---|---|
| `POST /registrar` cria `PASSAGEIRO` **ATIVO** imediatamente | SPEC-01 FR-AUT-08 | Passa a criar **`PENDENTE`** e disparar um **código OTP** (WhatsApp) para verificar o telefone; **só vira `ATIVO`** após a verificação (reusa a transição PENDENTE→ATIVO já existente do convite). Com **degradação graciosa** quando não há canal (RN-VER-07). |
| E-mail informado no cadastro **nunca é confirmado** | SPEC-01 | Passa a receber um **link mágico** de verificação por e-mail (reusa `TokenAtivacao` com `finalidade`); a conta funciona sem isso — e-mail é opcional (RN-AUT-02), mas fica marcada como **verificada** ao clicar. |
| Sem "esqueci a senha" | SPEC-01 §9 | Novo fluxo público `/esqueci-senha` → **código OTP** (WhatsApp) → `/redefinir-senha` define a nova senha. |
| Sem qualquer limite de tentativas | SPEC-01 §9 | O OTP tem **expiração curta + contador de tentativas + bloqueio** (RN-VER-04/05); requisições limitadas por identificador/IP (RN-VER-06). |
| Conta criada por Google não marca contato verificado | SPEC-08 | O **e-mail do Google já vem verificado** (`email_verified`): a conta nasce com `email_verificado_em` preenchido; ao completar o perfil com telefone (`/conta/completar`), dispara a verificação de **telefone** por OTP. |

Nada disso quebra o login por senha (SPEC-01) nem o Google (SPEC-08): **adiciona** camadas de
...
|---|---|---|---|
| **Verificar telefone** (no cadastro) | **Código OTP (6 díg.)** | **WhatsApp** | O telefone é o **identificador canônico** (Art. VIII); o WhatsApp **entrega de verdade** e casa com o bot da SPEC-11. |
...
- O caminho **feliz** (telefone + WhatsApp) **não depende do e-mail** — que segue stub até integrarmos
  ...
  quando o e-mail real existir (degrada como no-op até lá, coerente com a RN-WPP-02 da SPEC-10).
...
`status` (mesmo padrão do `perfil_incompleto` da SPEC-08). `NULL` = não verificado.
...
- **Telefone (OTP/WhatsApp):** no cadastro público e ao completar perfil (SPEC-08), dispara um código
...
- **Google (SPEC-08):** e-mail já chega **verificado** (`email_verified`); telefone é verificado quando
...
- **E-mail real** (`JavaMailSender`/SMTP) — é **pré-requisito** do canal e-mail, mas a integração de
  infra (segredo no deploy) é tarefa à parte, como a Evolution da SPEC-10 (ver §10 e §12).
- **Troca de telefone/e-mail** com reverificação obrigatória na tela de perfil (fica indicado, não
  detalhado aqui).
- **WhatsApp dos motoristas** (fora, como nas SPEC-10/11).
...
| **RN-VER-08** | Conta criada por **Google** (SPEC-08) nasce com **`email_verificado_em`** preenchido (`email_verified` do OIDC); o **telefone** é verificado por OTP quando informado em `/conta/completar`. |
...
| **RN-REC-05** | Uma conta **sem senha** (`hash_senha` nulo — ex.: passageiro auto-cadastrado pelo bot, SPEC-11) pode usar o "esqueci a senha" para **definir** a primeira senha (equivale ao "Acesso à plataforma"), desde que tenha telefone verificável. |
...
- **CA-VER-07 — Google já vem com e-mail verificado**
  *Dado* um primeiro login Google, *Então* a conta nasce com `email_verificado_em` preenchido (SPEC-08).
...
- **Conta sem senha** (bot, SPEC-11) usando "esqueci a senha" → **define** a primeira senha (RN-REC-05).
...
- **SPEC-01 §9** — marcar os três itens ("recuperação de senha", "verificação de e-mail/telefone",
  "bloqueio por tentativas") como **cobertos por SPEC-12** (o "bloqueio" apenas para os fluxos de
  código; lockout de login por senha continua fora — §5.4).
- **SPEC-08** — anotar que o e-mail do Google entra **verificado** e que `/conta/completar` passa a
  disparar a verificação de **telefone** (RN-VER-08).
- **SPEC-11** — "Acesso à plataforma" e o "esqueci a senha" convergem para **definir senha**; o
...
7. Integração **SPEC-08** (e-mail Google verificado; telefone em `/conta/completar`).
8. Testes (§15) + gate `docker build`; atualizar SPEC-01/08/11, roadmap e CLAUDE.md.

> **Pré-requisitos de infra:** o canal **WhatsApp** depende da **Evolution na VPS** (SPEC-10 §8); o
