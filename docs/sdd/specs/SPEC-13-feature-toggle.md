# SPEC-13 — Feature Toggle (flags de funcionalidade)
...
ambiente** (Google, WhatsApp `@ConditionalOnExpression`, flag da SPEC-12), mas **falta** um mecanismo
...
configs do WhatsApp (SPEC-11). Segue o ADR-10 (config dinâmica no banco).
...
pelo **município de origem** do passageiro (endereço — SPEC-07).
...
> são config (SPEC-11) — mesmo padrão, agora generalizado.
...
- **SPEC-11**: a flag `feature.bot_whatsapp` e o horário de atendimento (hoje persistido mas não
  aplicado) convergem — a janela pode passar a silenciar o bot.
- **SPEC-12**: os parâmetros do OTP passam a ser config (mantendo os defaults 10 min / 5 / 60 s).
