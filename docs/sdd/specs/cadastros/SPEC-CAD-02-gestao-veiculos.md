# SPEC-CAD-02 — Gestão de Veículos
...
- **FR-VEI-08** ✅ — `VeiculoService.listarAtivos()` expõe os veículos ativos ordenados por
  placa, para alimentar o select de viagens (ver SPEC-VIA-01).
...
- Não há sincronização automática de status (ex.: marcar `EM_VIAGEM` ao vincular a uma
  viagem) — é manual.
- Sem checagem de conflito de uso do mesmo veículo em viagens sobrepostas (ver SPEC-VIA-01, §8).
