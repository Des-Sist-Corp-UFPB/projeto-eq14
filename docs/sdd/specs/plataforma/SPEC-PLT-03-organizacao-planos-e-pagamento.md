# SPEC-PLT-03 — Organização, planos e pagamento (self-service do gestor)
...
| **Relacionada** | [SPEC-ACE-01](../acesso/SPEC-ACE-01-autenticacao.md) (auth/cadastro/convite), [SPEC-ACE-02](../acesso/SPEC-ACE-02-login-social-google.md) (OIDC), [SPEC-ACE-03](../acesso/SPEC-ACE-03-verificacao-de-contato-e-recuperacao-de-senha.md) (token/OTP), [SPEC-PLT-01](SPEC-PLT-01-feature-toggle.md) (**entitlement `municipios.pagamento_habilitado`** — RN-FLG-05), **[SPEC-PLT-02](SPEC-PLT-02-multi-ambiente-por-secretaria.md) §7 (gatilhos da Opção C)** · **ADR-20** (proposta) |
...
> gestor/secretaria — o que implicaria escolher a **Opção C** da SPEC-PLT-02."
...
multi-tenancy da SPEC-PLT-02. Três ajustes de modelagem, porém, são necessários — §2.
...
## 3. Isto implica a Opção C da SPEC-PLT-02? — **Sim, e a própria SPEC-PLT-02 já previa**

A [SPEC-PLT-02 §7](SPEC-PLT-02-multi-ambiente-por-secretaria.md) lista os gatilhos para abandonar a Opção A
...
| Gatilho previsto na SPEC-PLT-02 §7 | Esta proposta o dispara? |
|---|---|
...
Em vez de escrever `WHERE organizacao = ?` em cada consulta (o risco que a SPEC-PLT-02 §2 aponta como
...
3. **município do endereço** (SPEC-CAD-04) → organização que atende aquele município;
...
SPEC-PLT-01 (`municipios.pagamento_habilitado`) permanece válido e passa a conviver com a assinatura:
...
❓ **`configuracoes_sistema`**: hoje é global (sessão, WhatsApp, **flags da SPEC-PLT-01**). Com multi-tenant,
...
| Login Google/OIDC (SPEC-ACE-02) | 🟡 Quase nada | auto-provisão passa a exigir organização resolvida (D8) |
...
diferentes **enxergam os dados uns dos outros** (é o estado de hoje — SPEC-PLT-02 §1). Para demonstração
...
5. Atualizar [`docs/checklist.md`](../../../checklist.md) (pagamento ⬜ → ✅) e a
   [SPEC-PLT-02](SPEC-PLT-02-multi-ambiente-por-secretaria.md) §7 registrando que o gatilho foi acionado.
