# SPEC-VIA-03 — Solicitação de Transporte (Passageiro)
...
| **Relacionada** | [SPEC-VIA-02](SPEC-VIA-02-viagens-rotineiras-e-imprevistas.md) (linhas/designação), [SPEC-CAD-04](../cadastros/SPEC-CAD-04-endereco-do-passageiro.md) (endereço de embarque), [SPEC-ACE-01](../acesso/SPEC-ACE-01-autenticacao.md) (RBAC) |
...
viagens (SPEC-VIA-02): o gerente cria linhas e designa viagens; o passageiro as consome.
...
- Embarque a partir do **endereço** estruturado (SPEC-CAD-04) — o modelo já assume "busca no local",
...
| **RN-SOL-08** (✅ realizada na [SPEC-WPP-02](../whatsapp/SPEC-WPP-02-solicitacao-sob-demanda-e-onboarding-whatsapp.md)) | **Solicitação específica/sob demanda**: além de pedir uma viagem **já existente** (linha designada pelo gerente — escopo desta spec), o passageiro abre uma solicitação **sob demanda** informando **destino + data + horário + condições** (comorbidade/deficiência). Não casa com linha pré-definida: vira uma demanda que o gestor **avalia, aprova (designando viagem imprevista) ou recusa**. Implementada pela **SPEC-WPP-02** (migration V13: `solicitacoes_viagem.tipo`, `linha_programada` nullable, `cidade_destino`), via bot do WhatsApp + painel do gestor. |
...
5. Embarque a partir do **endereço** estruturado (SPEC-CAD-04).
