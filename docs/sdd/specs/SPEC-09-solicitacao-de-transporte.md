...
| **RN-SOL-08** (⏳ futuro) | **Solicitação específica/sob demanda**: além de pedir uma viagem **já existente** (linha designada pelo gerente — escopo atual), o passageiro poderá abrir uma solicitação **específica** informando requisitos: **data desejada**, necessidade de **veículo apropriado para PCD** (acessibilidade/cadeirante), **ambulância**, **acompanhante**, especialidade/observações clínicas, etc. Essa solicitação não casa com uma linha pré-definida: vira uma demanda **imprevista** (laranja) que o gestor avalia, aloca um veículo adequado e designa a viagem. Reaproveita a estrutura de `solicitacoes_viagem` (linha opcional) + campos de requisitos a definir; integra com a frota (`veiculos.possui_acessibilidade`, tipo `AMBULANCIA`). **Não implementado nesta etapa** — o foco atual é solicitar viagens já criadas pelo gerente. |
