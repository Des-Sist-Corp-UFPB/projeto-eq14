...
|---|---|---|---|
| Autenticação (login e-mail/telefone) | [SPEC-ACE-01](specs/acesso/SPEC-ACE-01-autenticacao.md) | ✅ | mensagem genérica, só ativos logam |
| Auto-cadastro de passageiro | [SPEC-ACE-01](specs/acesso/SPEC-ACE-01-autenticacao.md) | ✅ | papel `PASSAGEIRO`, status `ATIVO` |
| Logout + health `/ping` | [SPEC-ACE-01](specs/acesso/SPEC-ACE-01-autenticacao.md) | ✅ | `/ping` público |
| CRUD de usuários + papéis | [SPEC-CAD-01](specs/cadastros/SPEC-CAD-01-gestao-usuarios.md) | ✅ | soft-delete; busca multi-campo |
| CRUD de veículos | [SPEC-CAD-02](specs/cadastros/SPEC-CAD-02-gestao-veiculos.md) | ✅ | soft-delete; placa única |
| CRUD de cidades | [SPEC-CAD-03](specs/cadastros/SPEC-CAD-03-gestao-cidades.md) | ✅ | remoção física; seed na V3 |
| Viagens: criar/listar/excluir | [SPEC-VIA-01](specs/viagens/SPEC-VIA-01-gestao-viagens.md) | 🟡 | sem edição/transição de status |
| Painel inicial (totais) | (produto §4) | ✅ | contagens por repositório |
| Verificação de contato + recuperação de senha | [SPEC-ACE-03](specs/acesso/SPEC-ACE-03-verificacao-de-contato-e-recuperacao-de-senha.md) | ✅ | OTP/WhatsApp + link de e-mail, migration V14 |
| Observabilidade (OpenTelemetry) | [SPEC-OPE-01](specs/operacao/SPEC-OPE-01-observabilidade-opentelemetry.md) | ✅ | **em produção** (`dsc-eq14` no Grafana); agente auto (traces/métricas/logs) + 2 spans de negócio |
| **Feature toggle (flags + parâmetros + entitlement)** | [SPEC-PLT-01](specs/plataforma/SPEC-PLT-01-feature-toggle.md) | ✅ | **implementado** — `/admin/features`, modo de manutenção (503), bot on/off, `municipios.pagamento_habilitado` (**V15**) |
...
| **Multi-tenancy por secretaria (fase 1)** | [SPEC-PLT-02](specs/plataforma/SPEC-PLT-02-multi-ambiente-por-secretaria.md) | 🟡 | **implementada** — `organizacoes` + `vinculos` (**V16**), `ContextoTenant`, `/entrar/onde`. Decisão migrou da Opção A (silo) para a **B2** (schema por tenant + plano de controle no `public`) — **ADR-21/ADR-22**. **Aditiva**: sem vínculo, opera no tenant legado. Fases 2 e 3 abertas (**DT-33**) |
| Organização, planos e pagamento | [SPEC-PLT-03](specs/plataforma/SPEC-PLT-03-organizacao-planos-e-pagamento.md) | 🔵 | **proposta em avaliação** — cadastro de gestor com checkout (Mercado Pago) + `Organizacao`; aciona a **Opção C** da SPEC-PLT-02 |
...
|---|---|---|---|
| **DT-01** | Excluir cidade referenciada por viagem pode violar a FK | [SPEC-CAD-03 §7](specs/cadastros/SPEC-CAD-03-gestao-cidades.md) | ✅ Resolvido — `CidadeService` remove viagens em cascata + aviso no popup |
| **DT-02** | Sem trava contra excluir/suspender o **último gerente** ou a si mesmo | [SPEC-CAD-01 §9](specs/cadastros/SPEC-CAD-01-gestao-usuarios.md) | ✅ Resolvido — guarda do último gerente, bloqueio de auto-deleção, `solicitarSuspensao` |
| **DT-03** | Viagens não checam conflito de veículo/motorista sobrepostos | [SPEC-VIA-02 §6](specs/viagens/SPEC-VIA-02-viagens-rotineiras-e-imprevistas.md) | 🟡 Especificado na SPEC-VIA-02 (RN-VIA-08) — aguarda #21 |
| **DT-04** | `listarAtivos()` de veículos não filtra por `DISPONIVEL` | [SPEC-CAD-02 §8](specs/cadastros/SPEC-CAD-02-gestao-veiculos.md) | ✅ Resolvido — `listarDisponiveis()` filtra `DISPONIVEL` |
| **DT-05** | Sem edição/transição de status de viagem | [SPEC-VIA-02](specs/viagens/SPEC-VIA-02-viagens-rotineiras-e-imprevistas.md) | 🟡 Coberto pela SPEC-VIA-02 — aguarda #21 |
...
| **DT-08** | Data de viagem no passado não é bloqueada | [SPEC-VIA-02 §6](specs/viagens/SPEC-VIA-02-viagens-rotineiras-e-imprevistas.md) | 🟡 Especificado (RN-VIA-09, só imprevistas) — aguarda #21 |
| **DT-09** | `retorno_previsto` não coletado / tipo inconsistente | [SPEC-VIA-02 §2](specs/viagens/SPEC-VIA-02-viagens-rotineiras-e-imprevistas.md) | 🟡 Especificado — vira `horario_retorno` (LocalTime), ida/volta — aguarda #21 |
...
| **DT-12** | **Sem *lockout* do login por senha** — o OTP tem trava de tentativas (RN-VER-05), mas o `formLogin` aceita tentativas ilimitadas | [SPEC-ACE-03 §9](specs/acesso/SPEC-ACE-03-verificacao-de-contato-e-recuperacao-de-senha.md) | ⬜ Aberta — a auditoria já registra `LOGIN_FALHA`; falta bloquear (por conta e/ou IP) |
...
| **DT-15** | **SPEC-ACE-02 × SPEC-ACE-03**: conta criada por Google não passa pela verificação de contato (RN-VER-08) | [SPEC-ACE-03](specs/acesso/SPEC-ACE-03-verificacao-de-contato-e-recuperacao-de-senha.md) | ⬜ Aberta — o e-mail do Google já vem verificado; falta decidir o telefone |
| **DT-16** | **Janela de atendimento do WhatsApp** (início/fim): era persistida mas **nunca aplicada** — o bot respondia fora do horário e a tela dava a impressão contrária | [SPEC-WPP-02](specs/whatsapp/SPEC-WPP-02-solicitacao-sob-demanda-e-onboarding-whatsapp.md) | 🅿️ **Em standby — removida do sistema (2026-07-29)** por decisão do dono do produto: a janela só faz sentido **quando existir um serviço de suporte humano** por trás. Enquanto o atendimento é 100% bot, silenciá-lo à noite deixaria sem resposta justamente quem solicita viagem de madrugada. Campos, DTO, chaves de config e testes removidos; **reintroduzir junto do suporte humano** (aí a janela define quando o bot diz "um atendente responde a partir das 8h", e não quando ele emudece) |
...
| **DT-18** | **Cache de feature flags é por instância** (`FeatureFlagService`) — com várias réplicas, desligar uma flag não propaga | [SPEC-PLT-01 §4 D2](specs/plataforma/SPEC-PLT-01-feature-toggle.md) | ⬜ Aberta (aceita) — hoje roda 1 container por ambiente; revisar se houver réplica |
| **DT-21** | **Central de logs ainda não é por organização**: `/logs` mostra a trilha inteira do deploy. Com a [SPEC-PLT-03](specs/plataforma/SPEC-PLT-03-organizacao-planos-e-pagamento.md), cada secretaria deve ver **apenas os seus** eventos | (#19 + SPEC-PLT-03) | ⬜ Aberta — o ponto de leitura já é único (`AuditoriaService.listar`), então o filtro entra em um lugar só; o `log_auditoria` precisará da coluna `organizacao` (V17) |
...
| **DT-24** | **A cadeia do webhook falha em silêncio**: `EvolutionApiProvedor.registrarWebhook()` faz `return` mudo quando `APP_URL_PUBLICA` está vazia, e o `WhatsappWebhookController` responde **403 a tudo** quando `WHATSAPP_WEBHOOK_TOKEN` está vazio. Em ambos os casos o painel continua **verde ("Conectado")** — o status só prova o sentido CALADRIUS → Evolution. O sintoma para o gestor é "mando mensagem e não acontece nada", **sem um único log que explique o porquê** (diagnosticado em 2026-07-29) | [SPEC-WPP-01 §8](specs/whatsapp/SPEC-WPP-01-integracao-whatsapp.md) | ⬜ Aberta — `log.warn` ao pular o registro por falta de `APP_URL_PUBLICA` e no boot quando há provedor mas o token do webhook está vazio; no card de status do `/whatsapp`, distinguir "**recebimento não configurado**" de "não conectado" |

| **DT-33** | **Multi-tenancy parou na fase 1**: existem organizações e vínculos, mas **nenhuma tabela operacional está isolada** — quem tem vínculo com a secretaria A continua enxergando os dados da B, porque tudo ainda vive no mesmo schema. Os papéis também seguem vindo de `papeis_usuario` (globais), não do vínculo escolhido | [SPEC-PLT-02 §11](specs/plataforma/SPEC-PLT-02-multi-ambiente-por-secretaria.md) | ⬜ Aberta — **é a fase 2**: dividir `usuarios` em `identidades` + `membros`, ligar o `MultiTenantConnectionProvider` (`search_path`), criar a trilha Flyway `tenant/` e o provisionamento. **Critério de saída: teste de isolamento com dois schemas** (CA-MT-01) e teste de reset do pool (CA-MT-03). Enquanto isso não entrar, **nenhum cliente real pode ser onboardado** — para demonstração acadêmica é aceitável, desde que declarado |
| **DT-34** | **Privilégio de `CREATE` no banco não verificado**: a fase 2 precisa de `CREATE SCHEMA` no banco compartilhado da disciplina, e criar tabela (o que o Flyway já faz) **não** implica poder criar schema — são privilégios distintos | [SPEC-PLT-02 §9](specs/plataforma/SPEC-PLT-02-multi-ambiente-por-secretaria.md) | ⬜ Aberta — rodar `SELECT has_database_privilege(current_user, current_database(), 'CREATE');` no banco `eq14` **antes** de começar a fase 2. Se der `false`, a spec §9 já traz as três degradações (schemas pré-criados pelo DBA, VPS própria da equipe, ou Opção C) — nenhuma delas invalida o modelo, mas mudam o plano |

> **Pontos de contato da SPEC-PLT-02 com dívidas já abertas** (conferidos em 2026-08-04, nenhum
> bloqueia a fase 1): **DT-14** (tabelas dormentes) ganha destino — a spec §8.1 decide **não**
> migrá-las para a trilha do tenant; **DT-21** (logs por organização) passa a ser resolvida pela
> fase 2, com `log_auditoria` dentro do tenant e `log_plataforma` no `public`; **DT-29** (compose sem
> `UMAMI_*`) fica restrita ao *tier* dedicado; **DT-17** (home por papel) vai interagir com a escolha
> de contexto — a landing natural passa a ser a do vínculo escolhido; e o **Incremento C**
> (`escalas_motorista`, `perfis_*`) terá de nascer já na trilha `tenant/`. A **SPEC-PLT-03** também é
> afetada: a `organizacoes` saiu da V16 dela (já existe), sobrando `assinaturas` e `pagamentos`, e a
> **V17 (coluna `organizacao` em ~8 tabelas) deixa de existir**.

> **Nota sobre DT-03/05/08/09**: continuam marcadas como "aguarda #21", mas o essencial já entrou
> com a SPEC-VIA-02 (ciclo de status, painel semanal, designação). O que de fato falta é a **validação
...
> 1. **[SPEC-CAD-04](specs/cadastros/SPEC-CAD-04-endereco-do-passageiro.md) — Endereço do passageiro** (tabela
>    `enderecos` estruturada, **migration V8**) → vem **antes** das viagens.
> 2. **[SPEC-VIA-02](specs/viagens/SPEC-VIA-02-viagens-rotineiras-e-imprevistas.md) — Viagens rotineiras/imprevistas**
>    (`linhas_programadas` + `linha_dias` + alteração de `viagens`, **migration V9**). Decisões
>    confirmadas em [SPEC-VIA-02 §2.1](specs/viagens/SPEC-VIA-02-viagens-rotineiras-e-imprevistas.md) (ADR-12/13).

### Incremento A — Solicitações de transporte (passageiro) ✅
- **Entregue** em duas etapas: **por linha** ([SPEC-VIA-03](specs/viagens/SPEC-VIA-03-solicitacao-de-transporte.md), V11)
  e **sob demanda** ([SPEC-WPP-02](specs/whatsapp/SPEC-WPP-02-solicitacao-sob-demanda-e-onboarding-whatsapp.md), V13 —
...
- **Depende de**: Incremento A (solicitações) e SPEC-VIA-01 (viagens).
- **Resolve junto**: DT-04 (só alocar veículos disponíveis/compatíveis).

### Incremento B.1 — Aprovação/recusa do gestor (sob demanda) ✅
- **Entregue** na [SPEC-WPP-02](specs/whatsapp/SPEC-WPP-02-solicitacao-sob-demanda-e-onboarding-whatsapp.md): painel
...
- **Status**: **código implementado e testado (2026-07-14)** — [SPEC-WPP-01](specs/whatsapp/SPEC-WPP-01-integracao-whatsapp.md)
  (ADR-14), migration **V12** aplicada, 182 testes verdes. **Pendente**: subir a Evolution na
  **VPS da equipe** (SPEC-WPP-01 §8) e configurar as variáveis de ambiente no deploy
...
- **Entregue**: porta `ProvedorWhatsapp` + adaptador `EvolutionApiProvedor` (bean condicional),
  ...
  `SolicitacaoViagemService` (SPEC-VIA-03), e painel `/whatsapp` do gerente (QR + polling HTMX,
  status, desconectar, teste, últimas mensagens).

### Incremento E — Verificação de contato e recuperação de senha ✅ (core) / 🟡 (SPEC-ACE-02)
- **Objetivo**: verificar **telefone** (OTP/WhatsApp) e **e-mail** (link/e-mail) no cadastro, e um
  fluxo de **"esqueci a senha"** (OTP/WhatsApp) — modelo **híbrido** ([SPEC-ACE-03](specs/acesso/SPEC-ACE-03-verificacao-de-contato-e-recuperacao-de-senha.md), **ADR-16**).
- **Status**: ✅ **implementado e testado (2026-07-21)** — migration **V14**, **223 testes verdes**,
  ...
  degradação (RN-VER-07) e flag de DEV. **Pendente**: integração SPEC-ACE-02 (RN-VER-08) e **lockout do
...
- **Depende de**: canal WhatsApp = infra da Evolution (Incremento D); canal e-mail = integrar
  ...
  independe do e-mail.
- **Resolve junto**: os três itens da **SPEC-ACE-01 §9** (reset, verificação de contato, lockout dos
  códigos). Fica de fora o **lockout do login por senha** (nova DT sugerida).

### Incremento F — Observabilidade (OpenTelemetry) ✅ (em produção)
- **Status**: **em produção (2026-07-24)** — [SPEC-OPE-01](specs/operacao/SPEC-OPE-01-observabilidade-opentelemetry.md)
  ...
  **Ativado pelo portal da disciplina** (editor de `.env` que recria o container) — `dsc-eq14` recebendo
  no Grafana. Runbook + armadilhas da ativação em [SPEC-OPE-01 §11](specs/operacao/SPEC-OPE-01-observabilidade-opentelemetry.md).
...
- **Objetivo**: ligar/desligar funcionalidades em runtime pela área admin — **bot on/off**, **modo de
  ...
  — [SPEC-PLT-01](specs/plataforma/SPEC-PLT-01-feature-toggle.md), **ADR-17**.
...
- **Objetivo**: isolar **cada secretaria/cliente** no **seu próprio ambiente** (app + banco dedicado +
  subdomínio), sem um ver os dados do outro — [SPEC-PLT-02](specs/plataforma/SPEC-PLT-02-multi-ambiente-por-secretaria.md), **ADR-19**.
- **Decisão**: **Opção A (instância por cliente / silo)** — isolamento **físico**, **zero código de
  ...
  para quando exigir **painel único multi-secretaria**; ver [SPEC-PLT-02 §7](specs/plataforma/SPEC-PLT-02-multi-ambiente-por-secretaria.md)).
...
- **Objetivo**: **venda self-service** — quem se cadastra como gestor escolhe um plano, paga e só
  ...
  [SPEC-PLT-03](specs/plataforma/SPEC-PLT-03-organizacao-planos-e-pagamento.md), **ADR-20** (proposta).
...
- **Faseamento** (SPEC-PLT-03 §9): **fase 1** = `Organizacao`/`Assinatura`/`Pagamento` + checkout +
...
- **Impacto no login**: pequeno (o `UsuarioAutenticado` passa a carregar a organização; a tela de
  login não muda). O caro é a fase 2 — ver [SPEC-PLT-03 §6](specs/plataforma/SPEC-PLT-03-organizacao-planos-e-pagamento.md).
- **Aciona a Opção C** da [SPEC-PLT-02 §7](specs/plataforma/SPEC-PLT-02-multi-ambiente-por-secretaria.md) (auto-serviço de
  cadastro + painel único eram exatamente os gatilhos previstos).
- **Bloqueado por**: responder as decisões em aberto da SPEC-PLT-03 (planos/preços, resolução da
...
| Observabilidade | ✅ **Em produção** pela **[SPEC-OPE-01](specs/operacao/SPEC-OPE-01-observabilidade-opentelemetry.md)** (ADR-18): traces/métricas/**logs** via OpenTelemetry ao backend central (`dsc-eq14` no Grafana). |
